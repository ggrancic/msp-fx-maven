...
public class CajaDAOImpl extends ConexionMySQL implements CajaDAO {
    ...
    public ObservableList<Caja> listarConLimitYFecha(int inicio, int elementosPorPagina, Date fechaInicio, Date fechaFin) throws Exception {
        ...
        try {
            ...
            String sql = "select c.*, persona.*, em.id_empleado " +
                    "FROM cajas c " +
                    "JOIN empleados em ON c.responsable = em.id_empleado " +
                    "JOIN personas persona ON em.idpersona = persona.id_persona ";
            ...
            while (rs.next()) {
                ...
                caja.getResponsable().setNombre_usuario(rs.getString("persona.nombre") + " " + rs.getString("persona.apellido"));
